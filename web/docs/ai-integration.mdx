...
```text
Using the OpenAPI spec, generate a Python script using `requests` that runs the SIR simulation and prints the peak value of the Infected compartment from the median trajectory.
...
:::tip Safety filters
Some flagship models occasionally **flag conversations about epidemic models for safety reasons** and stops returning responces, even though the underlying work is a purely numerical, theoretical compartmental-model exercise. If this happens, framing it as examples above (theoretical/numerical work, textbook SIR / SEIR / SIS) is could be effective. Switching to smaller models (e.g. Claude Sonnet 4, ChatGPT 5.3 Instant) may also help.
:::
