...
A constant per-day hazard `rate` applied directly to each source compartment inside the window. This is mathematically identical to a spontaneous transition at `rate` (restricted to `target_age_groups` and the campaign window): every individual in a source compartment has a per-day probability of `1 - exp(-rate * dt)` of moving to the target.
...
- **Pre-step check, single-step overshoot possible.** The cap is evaluated at the start of each step, before the binomial draw. The campaign plateaus at approximately `threshold`, with up to one step's worth of doses of overshoot.
- **Dose-sink flows are not counted.** If a flow has `target: null` (a dose sink), doses delivered to it do not land in any named compartment and therefore do not count toward the cap. Avoid mixing sinks with `coverage` if precise tracking matters.
