...
If the request omits `initial_conditions`, V-SEIHR seeds ~0.05% of each age group split evenly between `Exposed` and `Infected` (~0.025% each); the rest goes to `Susceptible`, and the `_vax` branch starts at zero.
