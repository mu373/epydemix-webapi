...
All three transforms target an existing parameter via `target_parameter`, which must already be defined in `model.parameters`. The target must be a **source** parameter (scalar or age-varying) — [calculated parameters](./model/parameters.mdx#calculated-parameters) cannot be transform targets. Apply the transform to a source instead and let it propagate through the expression.
