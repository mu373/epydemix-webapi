...
The vaccination model presets ([`V-SEIR`](../model/presets/v-seir.mdx), [`V-SEIHR`](../model/presets/v-seihr.mdx)) are the built-in examples that use the vaccination feature: they expose a vaccinated branch of compartments with `VE_S` (and `VE_H` for V-SEIHR) efficacy parameters, and vaccination by default generates the flow `Susceptible → Susceptible_vax`.
...
```json5
{
  ...
  "vaccination": {
    // "flows" is optional on vaccination presets (defaults to S → S_vax pair); set it to override.
...
The block declares which compartments are involved in the rollout (`flows`) and which campaigns drive doses (`campaigns`). On the vaccination presets the default flow is `Susceptible → Susceptible_vax`; custom models must declare flows explicitly.

| Field | Required | Description |
|---|---|---|
| `flows` | for custom models | List of `{source, target}` compartment pairs. Vaccination presets default to `Susceptible → Susceptible_vax`. See [Flows](#flows) for shapes (including dose sinks) and validation. |
...
```json5
// 1. Perfect targeting (vaccination-preset default).
"flows": [{"source": "Susceptible", "target": "Susceptible_vax"}]
...
How much protection vaccination confers is a property of the **model**, not the rollout block. The vaccination presets ([`V-SEIR`](../model/presets/v-seir.mdx#vaccine-efficacy), [`V-SEIHR`](../model/presets/v-seihr.mdx#vaccine-efficacy)) define `VE_S` (against infection); V-SEIHR additionally defines `VE_H` (against hospitalization). See the preset pages for how they enter the dynamics. Custom models can mirror the same pattern with their own efficacy expressions.

## Inspecting the rollout

Each transitioning flow surfaces as a regular transition in the response. For the default vaccination-preset rollout, look for `Susceptible_to_Susceptible_vax` under `results.transitions.data`. Note that flows with dose sinks (`target: null`) emit no transition.
